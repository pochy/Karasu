...
| --------------------- | --------------------------------- |
| アプリ入口            | `src/main.ts`                     |
| YAML / 変換 / JWT / ツール | `src/features/yaml/`, `convert/`, `jwt/`, `devtools/` |
...
#### H 群 — 開発者ツール（Activity Bar 4 ページ）

| ID | 機能 | ステータス | 詳細 | 常時 CPU | 常時メモリ | 操作時 | I/O | GPU/DOM | 難度 | 工数 | 主な変更箇所 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **H1** | YAML エディタ | 実装済み | `.yaml` / `.yml` を開く・保存・整形。`yaml` は `import()` で遅延読込 | ◎ | ◎ | △ 入力時 validate | △ 開閉時 | ◎ | 2 | 1d | `features/yaml/`, `core/yaml.ts` |
| **H2** | JSON ↔ YAML 変換 | 実装済み | 左右ペイン。ボタンで片方向変換。ファイル I/O なし（スクラッチ） | ◎ | ◎ | △ 変換時 | ◎ | ○ | 2 | 0.5d | `features/convert/` |
| **H3** | JWT Viewer | 実装済み | ペースト → header / payload 表示。`exp` の有効期限表示。**署名検証なし** | ◎ | ◎ | ◎ | ◎ | ○ | 2 | 0.5d | `features/jwt/`, `core/jwt-decode.ts` |
| **H4** | 開発ツール集約 | 実装済み | Base64 / URL Encode・Decode / Hash（Web Crypto）/ UUID / URL Parser・Creator。入力 512KB 上限 | ◎ | ◎ | △ ボタン時 | ◎ | ○ 1 スクロール | 2 | 1d | `features/devtools/`, `core/devtools-utils.ts` |
| **H5** | TOML エディタ | 実装済み | `.toml` 開く・保存・整形。`smol-toml` は `import()` で遅延読込 | ◎ | ◎ | △ validate 時 | △ 開閉時 | ◎ | 2 | 1d | `features/toml/`, `core/toml.ts` |
| **H6** | JSON Schema 検証 | 実装済み | JSON モードで Schema パネル（ツールバー「Schema」）。`ajv` は検証時のみ `import()` | ◎ | ◎ | △ 検証時 | ◎ | ○ | 2 | 0.5d | `features/json/editor.ts`, `core/json-schema.ts` |
| **H7** | RegExp テスター | 実装済み | 開発ツールページ内。パターン + flags + テスト文字列 | ◎ | ◎ | ◎ | ◎ | ○ | 1 | 0.5d | `core/devtools-utils.ts` |
| **H8** | Unix タイムスタンプ変換 | 実装済み | 秒 / ミリ秒 / ISO 文字列を相互変換 | ◎ | ◎ | ◎ | ◎ | ○ | 1 | 0.5d | 同上 |
| **H9** | テキスト Diff | 実装済み | 行単位 LCS diff（`-` / `+` / 空白） | ◎ | ◎ | △ 比較時 | ◎ | ○ | 2 | 0.5d | 同上 |
| **H10** | カラーコード変換 | 実装済み | `#hex` / `rgb()` / `rgba()` 相互変換 + スウォッチ | ◎ | ◎ | ◎ | ◎ | ○ | 1 | 0.5d | 同上 |

**H 群の UX**: Activity Bar に Markdown / JSON / CSV に続きツール系アイコンを追加。スクラッチページは「開く」「別名で保存」を非表示。`suspend()` で内容クリア。
...
| H1     | H    | YAML エディタ            | 実装済み   | 2    | 1d      | ◎                | △                  | 2            |
| H2     | H    | JSON ↔ YAML              | 実装済み   | 2    | 0.5d    | ◎                | △                  | 2            |
| H3     | H    | JWT Viewer               | 実装済み   | 2    | 0.5d    | ◎                | ◎                  | 2            |
| H4     | H    | 開発ツール集約           | 実装済み   | 2    | 1d      | ◎                | △                  | 2            |
| H5     | H    | TOML エディタ            | 実装済み   | 2    | 1d      | ◎                | △                  | 2            |
| H6     | H    | JSON Schema 検証         | 実装済み   | 2    | 0.5d    | ◎                | △                  | 2            |
| H7〜H10 | H   | RegExp / Timestamp / Diff / Color | 実装済み | 1〜2 | 0.5d | ◎ | ◎〜△ | 2 |
